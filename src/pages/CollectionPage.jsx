...
export default function CollectionPage({ categoryName = "T-shirts técnicas", title }) {
  ...
  // rótulo do filtro de tamanho adapta-se à categoria (roupa vs garrafas/acessórios)
  const isCapacity = /garraf|acess/i.test(categoryName);
  const sizeLabel = isCapacity ? "Capacidade" : "Tamanho";
    ...
    <Layout>
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "32px 24px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "220px 1fr", gap: 32, alignItems: "start" }}>
          ...
          <main>
            {loading ? (
              <div style={{ color: T.muted, padding: 40, textAlign: "center" }}>A carregar produtos...</div>
            ) : error ? (
              <div style={{ color: T.danger, padding: 40, textAlign: "center" }}>Erro ao carregar produtos: {error}</div>
            ) : filtered.length === 0 ? (
